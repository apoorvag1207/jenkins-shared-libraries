...
def call(Map config = [:]) {
    ...
    withCredentials([usernamePassword(
        ...
        sh """
            ...
            sed -i "s|image: apoorva1206/easyshop-app:.*|image: apoorva1206/easyshop-app:${imageTag}|g" ${manifestsPath}/08-easyshop-deployment.yaml
            
            # Update migration job if it exists
            if [ -f "${manifestsPath}/12-migration-job.yaml" ]; then
                sed -i "s|image: apoorva1206/easyshop-migration:.*|image: apoorva1206/easyshop-migration:${imageTag}|g" ${manifestsPath}/12-migration-job.yaml
            fi
